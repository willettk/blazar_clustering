...
The parent blazar sample was assembled from available compilations in the literature. The primary source for blazar identification comes from the $5^\mathrm{th}$ edition of the multi-frequency catalog Roma-BZCAT\footnote{\url{http://www.asdc.asi.it/bzcat/}} \citep{mas15}. BZCAT compiles known blazars from numerous surveys, primarily in radio and X-ray wavelengths. Photometric information includes the radio flux density at 1.4~GHz and the X-ray flux between 0.1--2.4~keV, as well as its $r^\prime$-band magnitude as determined from SDSS. We supplement BZCAT with SDSS optically-selected BL~Lac \citep{plo10} and FSRQ candidates \citep{che09a}, along with gamma-ray selected blazars from TeVCat \citep{hor08}. 

\begin{figure}
%\includegraphics[width=3.5in]{blazar_zhist.eps}
\includegraphics[width=3.5in]{figures/bzcat5/blazars_zhist.png}
\caption{Redshift distribution for the 657~blazars within the footprint of SDSS~DR12 at $0.043<z<0.75$. BL~Lacs and BL~Lac candidates dominate the population at $z<0.5$, while FSRQs are more common at higher redshifts. 
\label{fig:blazar_zhist}}
\end{figure}

After removing duplicates, the combined catalogs yielded 3,631~blazars; of those, 2,844 have a published redshift. Figure~\ref{fig:blazar_zhist} shows their redshift distribution {\note (change this figure so that it shows ALL blazars, not just those we ultimately included?)}. The distribution varies significantly by blazar sub-type; FSRQs comprise 95\% of the blazars at $z>1$, with a mean redshift of $\langle z\rangle=1.4$. The BL~Lacs have a much lower average redshift, at $\langle z\rangle=0.45$. Unclassified blazars \citep[``QSO candidates'', as defined by][]{mas09} have a mean redshift of {\note$X.XX$}; however, a K-S test confirms {\note (to what $d$-value?)} that its redshift distribution is more consistent with BL~Lacs than QSOs. 

We then take the positions of each blazar and searched the SDSS DR12 catalog \citep{ala15} for entries in the \texttt{PhotoObj} table for nearby neighbors, and restricting results to objects identified by the SDSS pipeline as galaxies. The angular radius of the search for each blazar is set at a projected distance of 500~kpc at the redshift of the blazar. The apparent magnitude down to which a galaxy is considered for counting is the brighter of: $M_r^* + 2 > M_{r,lim}$, where $M_{r,lim}$ is the absolute magnitude corresponding to the $r$-band completeness limit of the SDSS at the distance of the blazar. 
...
Several additional constraints further lower the number of blazars on which clustering analysis can be performed. Although BZCAT is an all-sky catalog, only $\sim60\%$ of the objects fall within the SDSS footprint. Of those blazars, we analyze data only for blazars in the redshift range $0.043<z<0.75$. The lower limit is due to the practical consideration of searching large areas of sky within a projected radius of 500~kpc, which corresponds to an angular size of 10\arcmin. This eliminated {\note 1.5\%} of the blazars within the SDSS footprint. The upper redshift limit is set by the sensitivity of the SDSS; blazars at high redshifts will have fewer neighboring galaxies that can be detected with SDSS. For high redshift objects computing the \bgb{} coefficient will (statistically) include no neighbors that are physically associated with the blazar, and so the clustering coefficient approaches that of the background sky. The upper redshift limit is thus chosen so that our absolute counting magnitude corresponds to an $M^*$ galaxy at $z=0.75$, eliminating another {\note 41\%} of the SDSS blazars. Finally, to ensure that measurements of \bgb{} are not dominated by low number counts, we analyze only blazars with at least 10~neighboring galaxies detected. The final sample contains 657~blazars, with 528~BL~Lacs, 98~FSRQs, and 131~unclassified blazars. 
...
\includegraphics[width=3.5in]{figures/bzcat5/bgb_hist.png}
...
Among the objects studied are {\note XX} objects whose blazar type is defined by BZCAT as ``uncertain'' or galaxy-dominated BL~Lacs \citep{mas12}. Discuss possibility of matching these to BL~Lac/FSRQ, information available on selection method, and whether the environmental technique might be used as a diagnostic for blazar type. Probably due to optical lines?
...
\includegraphics[width=3.5in]{figures/bzcat5/bgb_redshift.png}
...
\startdata
Blazars                                   & 657         & 195            & 1232           & 1,2,3,4 \\
\hspace{10 pt} BL~Lac                     & 262         & 82             & 206            &         \\
\hspace{10 pt} FSRQ                       & 137         & 55             & 155            &         \\
\hspace{10 pt} Uncertain                  & 258         & 382            & 1938           &         \\
Radio galaxies                            & 283         & 131            & 410            & 5       \\
\hspace{10 pt} FR I                       & 55          & 138            & 598            &         \\
\hspace{10 pt} FR II                      & 184         & 133            & 372            &         \\
\hspace{10 pt} Compact                    & 44          & 111            & 258            &         \\

Radio galaxies (optical counterparts)     & 258         & 269            & 357            & 6       \\
\hspace{10 pt} Complex                    & 111         & 321            & 357            &         \\
\hspace{10 pt} Compact                    & 37          & 282            & 434            &         \\
\hspace{10 pt} Resolved                   & 110         & 211            & 323            &         \\

Optical galaxies                          & 624         & 125            & 335            & 7       \\
\hspace{10 pt} Elliptical                 & 540         & 145            & 329            &         \\
\hspace{10 pt} Spiral                     & 84          & $-2$           & 348            &         \\

\enddata
\tablerefs{(1) - \citet{mas15}; (2) - \citet{plo10}; (3) - \citet{hor08}; (4) - \citet{che09a}; (5) - \citet{gen08}; (6) - \citet{kim08}; (7) - \citet{lin11}}
\tablecomments{``Uncertain'' blazar types include both BL~Lac candidates and galaxy-dominated BL~Lacs.}
\end{deluxetable*}
